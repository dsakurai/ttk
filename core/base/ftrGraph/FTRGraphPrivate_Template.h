...
#pragma omp critical(stats)
    ...
    std::pair<valence, valence> FTRGraph<ScalarType>::getLinkNbCC(
      ...
      for(idCell t = 0; t < nbTri; ++t) {
        ...
        if(curDownSide == 2) {
          localForests.down.insertEdge(
            downSide[0], downSide[1], 0, nullSuperArc);
        } else if(curUpSide == 2) {
      ...
      const valence up
        = mapNeighUp.size() - (oldUpCC - localForests.up.getNbCC());
      return {down, up};
    }
...
#pragma GCC diagnostic pop
    ...
    idSuperArc FTRGraph<ScalarType>::mergeAtSaddle(
      ...
      if(compVect.size() < 2) {
        std::cerr << "[FTR]: merge at saddle with only one lower CC"
                  << std::endl;
      }
#endif
